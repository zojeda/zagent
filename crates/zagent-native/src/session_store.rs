...
const MIGRATION_SQL: &str = include_str!("../migrations/001_session_schema_v3.surql");
...
async fn ensure_schema(db: &Surreal<Any>) -> Result<()> {
    db.query(MIGRATION_SQL)
        .await
        .map_err(|e| zagent_core::Error::session(format!("Failed to initialize schema: {e}")))?;
    Ok(())
}
