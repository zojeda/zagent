...
/// 1. Terminal layer: compact, colored, INFO+ (overridable via RUST_LOG)
...
pub fn init_tracing(log_dir: &str, verbose: bool) -> TracingGuard {
    ...
    // Terminal layer — human-readable, compact format
    ...
    // JSON file layer — structured, everything at TRACE level
    let file_appender = rolling::daily(log_dir, "zagent.log");
...
fn default_app_filter(level: &str) -> EnvFilter {
    // Keep default noise low by excluding transport internals.
    let directives = format!(
        "zagent={level},zagent_core={level},zagent_native={level},hyper=off,h2=off,reqwest=off,tower=off,tonic=off,rustls=off"
    );
...
fn build_otel_tracer() -> Option<sdktrace::Tracer> {
    ...
    let service_name = std::env::var("OTEL_SERVICE_NAME").unwrap_or_else(|_| "zagent".to_string());
